...
var ucebolunenlerintoplami = ucetambolunenler.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
	...
	var besyuzdenkucuksayilar = sayilar.filter(number => number < 500);
 ...
 var siralisayilar = besyuzdenkucuksayilar.sort((a, b) => a - b);
	...
	// First loop to count the occurrences of each number
	for (var i = 0; i < sayilar.length; i++) {
	  var number = sayilar[i];
	...
	// Second loop to create the string for each duplicate number and its frequency of occurrence
	var tekraredensayilar = [];
	for (var number in occurrences) {
	  if (occurrences[number] > 1) {
		var tekrarString = `${number} sayısı ${occurrences[number]} kere tekrar edilmiştir`;
